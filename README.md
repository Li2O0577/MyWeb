...
- 全量数据查看：数据加载页支持查看所有列，并以 50/100/200/500 行分页浏览完整数据集。
...
- 训练任务状态：任务历史持久化到 SQLite，前端模型页可查看最近训练任务、成功/失败、耗时和结果版本；回归、分类、决策树、聚类和 MLP 训练均支持 `?async=1` 后台任务。
- 本地账号：账号和登录 cookie session 持久化到 SQLite，支持注册、登录、退出；未登录不能访问 `/api/*` 业务接口。
- 用户隔离：上传 session、数据处理历史、模型版本、模型激活状态和训练任务列表均按账号隔离。
- 任务队列：训练任务进入进程内执行队列，支持全局并发限制、单用户并发限制和排队状态；服务重启后历史记录仍可查询，未完成任务会标记为“因服务重启中断”。
...
- 账号管理：首个账号或迁移后最早创建的账号成为管理员；管理员可设置角色、启用/禁用账号，并查看登录和权限变更审计日志。
...
- 当前使用单机 SQLite，适合单进程或内网部署；多进程/多服务器部署需要迁移到 PostgreSQL 等共享数据库。
- 任务执行队列仍在单个 Flask 进程内，不能跨进程调度或在重启后续跑；正式部署建议使用独立任务系统。
- 运行中任务只能“协作式取消”：不能强制终止正在执行的训练线程，只能阻止结果落库并标记为已取消。
- 还没有细粒度业务权限、数据库级限流、磁盘/CPU/GPU 配额和完整自动清理策略。
...
│   ├── auth_store.py             # SQLite 账号和持久登录 cookie session
│   ├── database.py               # SQLite schema 和连接管理
│   ├── session_store.py          # session 持久化和恢复
│   ├── task_store.py             # 后台任务队列、状态持久化和取消
│   ├── routes/                   # Flask API 路由
│   ├── services/                 # 数据处理、可视化、模型、LLM 业务逻辑
│   ├── models/                   # 模型注册表和模型版本文件
│   ├── auth/                     # 旧版 users.json，仅用于自动迁移
│   ├── runtime/                  # SQLite 数据库，运行时生成，不提交
...
- `GET /api/admin/audit-logs`（管理员）
- `GET /api/health`
- `POST /api/data/upload`
- `DELETE /api/data/<sid>`
- `GET /api/data/<sid>/profile`
- `GET /api/data/<sid>/rows?page=1&page_size=100`
...
`.env.example` 是变量清单。当前启动脚本不会自动读取 `.env`；需要在 PowerShell 中设置环境变量，或在部署平台中配置。
...
| `MYWEB1_DB_PATH` | `backend/runtime/myweb1.db` | 账号、登录 session 和任务历史 SQLite 文件；相对路径以项目根目录解析 |
| `MYWEB1_MAX_AUDIT_LOGS` | `10000` | 最多保留的审计事件数量，超出后删除最旧记录 |
...
- `backend/runtime/myweb1.db` 存放账号、登录 session 和任务历史，属于关键运行数据，应定期备份且不应提交。
- 旧版 `backend/auth/users.json` 会自动导入 SQLite，但不会被删除，可在确认迁移和备份后自行归档。
...
测试里会打印两段预期异常日志，分别来自“不安全自定义表达式”和“非法热力图配置”的负向用例；只要最终显示 `OK` 就是通过。当前 smoke tests 覆盖了 SQLite 账号迁移、持久登录、管理员权限、账号禁用、审计日志、任务恢复、资源配额、任务取消、上传、处理历史、可视化、模型和 LLM 基础接口。
...
3. 将当前管理员/普通用户两级权限扩展为按资源和操作划分的细粒度权限，并补充磁盘、CPU、GPU 和模型存储配额。
4. 将进程内任务执行迁移到独立 worker，支持硬取消、进度事件和多进程部署。
