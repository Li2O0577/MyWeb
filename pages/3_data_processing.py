...
with st.expander("8. PCA 降维"):
    ...
    if len(pca_candidate_cols) < 2:
        ...
        pca_cols = st.multiselect("用于降维的数值列", pca_candidate_cols, default=pca_candidate_cols, key="pca_cols_1")
        max_dim = len(pca_cols) if pca_cols else 1
        n_components = st.slider("降维维度", 1, max_dim, min(2, max_dim), key="pca_dim_1")
...
with ctrl4:
    st.download_button(label="导出CSV", data=df.to_csv(index=False).encode("utf-8"), file_name="processed_data.csv", mime="text/csv", use_container_width=True, key="btn_export")
