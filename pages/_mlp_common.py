...
def save_model_files(model, scaler, config_dict, model_path, scaler_path, config_path):
    """Persist model state_dict, scaler, and JSON config to disk."""
...
def clear_model_files(paths, session_keys):
    """Remove saved model files and clear session_state keys."""
    for p in paths:
        if os.path.exists(p):
            os.remove(p)
